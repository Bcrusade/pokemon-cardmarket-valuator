...
def test_parse_realistic_cardmarket_listing_fixture() -> None:
    html = (FIXTURES / "cardmarket_listings_valid.html").read_text()
    rows = parse_listing_rows(html, "https://provider.example/result-2")

    assert len(rows) == 3
    assert rows[0].price == 4.5
    assert rows[0].condition == "NM"
    assert rows[0].language == "English"
    assert rows[0].seller_name == "sellerAlpha"
    assert rows[0].quantity == 1

    assert rows[1].price == 5.0
    assert rows[1].condition == "EX"
    assert rows[1].language == "German"
    assert rows[1].seller_name == "sellerBeta"
    assert rows[1].quantity == 2

    assert rows[2].price == 99999.0
    assert rows[2].flags == ["non-comparable"]
